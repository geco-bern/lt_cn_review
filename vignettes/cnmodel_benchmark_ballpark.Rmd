...
gg1 <- df2 |> 
  ggplot(aes(lma, ..density..)) +
  geom_density(fill = "#777055ff", color = NA, alpha = 0.5) +
  theme_classic() +
  geom_point(aes(x = 150, y = 0), size = 5, color = "#29a274ff") +
  labs(x = "LMA") +
  coord_cartesian(clip = 'off') +
  scale_y_continuous(expand = c(0, 0))

gg2 <- df2 |> 
  ggplot(aes(nmass, ..density..)) +
  geom_density(fill = "#777055ff", color = NA, alpha = 0.5) +
  theme_classic() +
  # geom_point(aes(x = 150, y = 0), size = 5, color = "#29a274ff") +
  labs(x = expression("N"[mass])) +
  coord_cartesian(clip = 'off') +
  scale_y_continuous(expand = c(0, 0))

gg3 <- df2 |> 
  ggplot(aes(narea, ..density..)) +
  geom_density(fill = "#777055ff", color = NA, alpha = 0.5) +
  theme_classic() +
  # geom_point(aes(x = 150, y = 0), size = 5, color = "#29a274ff") +
  labs(x = expression("N"[area])) +
  coord_cartesian(clip = 'off') +
  scale_y_continuous(expand = c(0, 0))

gg4 <- df2 |> 
  ggplot(aes(vcmax, ..density..)) +
  geom_density(fill = "#777055ff", color = NA, alpha = 0.5) +
  theme_classic() +
  # geom_point(aes(x = 150, y = 0), size = 5, color = "#29a274ff") +
  labs(x = expression("V"[cmax])) +
  coord_cartesian(clip = 'off') +
  scale_y_continuous(expand = c(0, 0))
