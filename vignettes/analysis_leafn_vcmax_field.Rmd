...
```{r}
df_trobit <- read_csv("~/data/leaf_traits/combined_leaf_traits_updated.csv")

df_trobit_sitemean <- aggregate(df_trobit,by=list(df_trobit$lon_2,df_trobit$lat_2), FUN=mean, na.rm=TRUE) #site-mean
...
df_trobit_sitemean <- df_trobit_sitemean %>%
  mutate(
    log_narea = log(narea),
    log_nmass = log(nmass),
    log_leafCN = log(leafCN),
    log_LMA = log(lma),
    log_Vcmax25 = log(Vcmax25),
    log_cmass = log(cmass),
    log_carea = log(carea)
    )
