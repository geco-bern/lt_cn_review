...
```{r}
linmod_vcmax <- lm(vcmax_obs ~ lnD + mgdd0 + lnppfd + alpha + gdday + soil_ph + soil_cn + elv, data = df)
linmod_narea <- lm(Narea ~     lnD + mgdd0 + lnppfd + alpha + gdday + soil_ph + soil_cn + elv, data = df)
linmod_nmass <- lm(Nmass ~     lnD + mgdd0 + lnppfd + alpha + gdday + soil_ph + soil_cn + elv, data = df)
linmod_lma   <- lm(LMA ~       lnD + mgdd0 + lnppfd + alpha + gdday + soil_ph + soil_cn + elv, data = df)

library(visreg)
gg_vcmax <- visreg(linmod_vcmax, "soil_cn", gg = TRUE)
gg_narea <- visreg(linmod_narea, "soil_cn", gg = TRUE)
gg_nmass <- visreg(linmod_nmass, "soil_cn", gg = TRUE)
gg_lma   <- visreg(linmod_lma, "soil_cn", gg = TRUE)
