...
use_vars <- c("Vcmax", "Narea", "Nmass", "LAI", "Leaf area per plant", "Leaf biomass")
...
df <- df %>% 
  ...
  # ## exclude data where N-fertilisation levels are very large (greater than 500 kg ha-1 yr-1 = 50 g_n_per_m2_and_year)
  # filter(ndep_kg_ha_yr <= 500) |> 
...
gg_nfert_liang <- ggplot(data = df_plot, 
       aes(x = response, y = logr)) +
  geom_jitter( color = rgb(0,0,0,0.4), aes( size = 1/logr_se ), position = position_jitter(w = 0.2, h = 0), show.legend = FALSE ) +
...
gg_nfert_liang
ggsave(paste0(here::here(), "/fig/metaanalysis_nfert_liang.pdf"), width = 8, height = 6)
ggsave(paste0(here::here(), "/fig/metaanalysis_nfert_liang.png"), width = 8, height = 6)
