...
Load stuff for creating the references table. Get mesi-db data and source code for functions loaded below from https://github.com/MESI-organization/mesi-db. 
```{r}
library(xtable)
source("~/mesi-db/R/create_table_latex.R")
```
...
df0 <- df %>%
  ...
  # filter(experiment_type == "field") %>%   # remove this filter (Kevin, email 29.11.2023)
...
saveRDS(df_box, file = here::here("data/df_box_mesi_co2.rds"))
```
...
Create table with experiments information.
```{r}
df_experiments_co2 <- df5 |> 
  select(exp, myvar) |> 
  left_join(
    df |> 
      select(exp, myvar = response, citation),
  ...
  group_by(exp) |> 
  summarise(
    myvar = paste0(unique(myvar), collapse = ", "),
    citation = paste0(unique(citation), collapse = ", ")
  ) |> 
  rename(
    Experiment = exp,
...
create_table_latex(
  df_experiments_co2 |> 
    slice(1:11), 
  caption = "Experiments, MESI, CO2", 
  filn = here("manuscript/df_experiments_co2_01.tex"),
  align = c("p{0.1cm}", "p{3cm}", "p{4cm}", "p{4cm}")
  )
...
create_table_latex(
  df_experiments_co2 |> 
    slice(12:22), 
  caption = "Experiments, MESI, CO2", 
  filn = here("manuscript/df_experiments_co2_02.tex"),
  align = c("p{0.1cm}", "p{3cm}", "p{4cm}", "p{4cm}")
  )

# # Read the .tex file
# lines <- readLines(here("manuscript/df_experiments_co2.tex"))
# 
...
# writeLines(
#   modified_lines, 
#   here("manuscript/df_experiments_co2.tex")
#   )

write_csv(
  df_experiments_co2,
  file = here::here("data/df_experiments_co2.csv")
)
```
...
```{r}
sites_mesi <- df |> 
  mutate(eCO2_magnitude = c_t - c_c) |>
  select(exp, citation, site, lat, lon, elevation, ecosystem_type, experiment_type,
         fumigation_type, eCO2_magnitude, sampling_year) |> 
  distinct() |> 
  filter(exp %in% unique(df5$exp)) |> 
  group_by(site, exp, lon, lat, elevation, ecosystem_type, experiment_type,
           fumigation_type) |> 
  summarise(eCO2_magnitude = list(eCO2_magnitude),
            citation = list(citation),
            sampling_year = list(na.omit(sampling_year)))

saveRDS(sites_mesi, file = paste0(here::here(), "sites_mesi.rds"))

sites_mesi

# Convert the 'citation' column to a character vector
sites_mesi$eCO2_magnitude <- sapply(sites_mesi$eCO2_magnitude, function(x) paste(x, collapse = ", "))
sites_mesi$citation <- sapply(sites_mesi$citation, function(x) paste(x, collapse = ", "))
sites_mesi$sampling_year <- sapply(sites_mesi$sampling_year, function(x) paste(x, collapse = ", "))

# Function to remove duplicates within a string
remove_duplicates <- function(text) {
  elements <- unlist(strsplit(text, ", "))  # Split string by comma and space
  unique_elements <- unique(elements)       # Remove duplicates
  unique_string <- paste(unique_elements, collapse = ", ")  # Combine unique elements
  return(unique_string)
}

# Apply the duplicate removal function to 'citation' and 'sampling_year' columns
sites_mesi$eCO2_magnitude <- sapply(sites_mesi$eCO2_magnitude, remove_duplicates)
sites_mesi$citation <- sapply(sites_mesi$citation, remove_duplicates)
sites_mesi$sampling_year <- sapply(sites_mesi$sampling_year, remove_duplicates)

# Export the modified 'sites_mesi' data frame as a CSV file
write_csv(sites_mesi, file = here::here("data/eCO2_experiments.csv"))
```
...
for (ivar in unique(df2$myvar)){
  ...
  label <- df_labels |> 
    ...
    pull(mylabl)
  pdf(
    file = here(paste0("fig/forestplot_mesi_co2_", ivar, ".pdf")),
    width = 8,
...
df5_anpp_bnpp %>% 
  ...
  ggrepel::geom_text_repel(size = 3, 
                           ...
                           segment.alpha = 0, 
                           color = "grey50") +
  labs(x = "Log RR of root NPP", y = "Log RR of ANPP", 
...
gg_eco2_cpl_agb_bgb_mesi <- df5_agb_bnpp %>% 
  ...
  labs(x = "Normalised log RR of BGB", 
       ...
       title = expression("Response to CO"[2]),
       subtitle = "MESI data") +
  theme_classic()

saveRDS(gg_eco2_cpl_agb_bgb_mesi, 
        file = paste0(here::here(), "/data/gg_eco2_cpl_agb_bgb_mesi.rds"))
...
XXXXXXXXXX
This is now done in `vignettes/analysis_mesi_nutnet_nfert.Rmd`, combining NutNet and MESI data for N-fertilisation.
XXXXXXXXXX
...
df0 <- df |> 
  ...
  ## N fertilisation experiments only (KVS edit: npk = '_100' = N addition vs Control only)
  filter(treatment == "f")|> 
  filter(npk == "_100")|> 
...
df_boxb <- purrr::map_dfr(outb, "df_box") |>
  separate(var,
           c("var", "nfert_lev"),
           sep = "_XXX_")
saveRDS(df_boxb, file = here::here("data/df_boxb_mesi_nfert.rds"))
```
...
write_csv(df4, file = here::here("data/n_experiments.csv"))
```

Available experiments by variable (code added by Kevin Van Sundert)
```{r}
sites_mesi_n <- df2 |> 
  select(exp, citation, site, lat, lon, elevation, ecosystem_type, experiment_type,
        n_t, sampling_year) |> 
  distinct() |> 
  filter(exp %in% unique(df4$exp)) |> 
  group_by(site, exp, lon, lat, elevation, ecosystem_type, experiment_type) |> 
  summarise(n_t = list(na.omit(n_t)),
            citation = list(citation),
            sampling_year = list(na.omit(sampling_year)))

saveRDS(sites_mesi_n, file = paste0(here::here(), "sites_mesi_n.rds"))

sites_mesi_n

# Convert the n_magnitude, citation, and sampling year  columns to character vectors
sites_mesi_n$n_t <- sapply(sites_mesi_n$n_t, function(x) paste(x, collapse = ", "))
sites_mesi_n$citation <- sapply(sites_mesi_n$citation, function(x) paste(x, collapse = ", "))
sites_mesi_n$sampling_year <- sapply(sites_mesi_n$sampling_year, function(x) paste(x, collapse = ", "))
...
# Apply the duplicate removal function to 'n_t', 'citation' and 'sampling_year' columns
sites_mesi_n$n_t <- sapply(sites_mesi_n$n_t, remove_duplicates)
sites_mesi_n$citation <- sapply(sites_mesi_n$citation, remove_duplicates)
sites_mesi_n$sampling_year <- sapply(sites_mesi_n$sampling_year, remove_duplicates)

# Export the modified 'sites_mesi_n' data frame as a CSV file
write_csv(sites_mesi_n, file = here::here("data/n_experiments.csv"))
```
