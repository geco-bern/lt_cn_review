...
## create own variables, grouping available variables
use_vars <- c("asat", "vcmax", "leaf_n", "leaf_cn", "agb", "bgb", "lai", "root_shoot_ratio")
df2 <- df1 %>% 
  filter(exp %in% use_exp) %>% 
  mutate(myvar = ifelse(response %in%  c("agb", "agb_c"),   "agb", response)) %>% 
  mutate(myvar = ifelse(response %in%  c("bgb", "bgb_c"),   "bgb", response)) %>% 
  mutate(myvar = ifelse(response %in%  c("lai", "lai_max"), "lai", response)) %>% 
  filter(myvar %in% use_vars)

  # c("asat", "vcmax", "jmax", "leaf_n", "leaf_cn", "fine_root_cn", "bgb_cn", "wood_cn", "lai", "lai_max", "anpp", "anpp_leaf", "anpp_woody", "root_n_uptake", "soil_solution_mineral_n", "soil_nh4-n", "soil_nh4", "soil_no3-n", "soil_no3", "root_shoot_ratio", "root_production", "root_growth")
```
...
df4 %>%
  ...
  geom_crossbar( data = df_box, aes(x = myvar, y = middle, ymin = ymin, ymax = ymax), fill = "tomato", color = "tomato4", alpha = 0.6, width = 0.5 ) +
  geom_hline( yintercept = 0.0, size = 0.5 ) +
  labs(x = "Variable", y = "Log Response Ratio", size = expression(paste("Error"^{-1}))) +
  coord_flip() +
  ylim(-1, 1) +
  labs(title = "MESI data")
```
