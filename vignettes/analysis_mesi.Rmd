...
out  <- purrr::map(as.list(use_vars), ~analyse_meta(df5 %>% rename(var = myvar), nam_target = .))
names(out) <- use_vars
...
```{r}
df_labels <- tibble(
  myvar = rev(c("asat", "gpp", "vcmax", "jmax", "gs", "leaf_n_area", "leaf_n_mass", "leaf_cn", "lai", "leaf_area", "leaf_biomass", "anpp", "agb", "root_production", "bgb", "root_shoot_ratio", "n_uptake", "n_inorg")),
  mylabl = rev(c(
    expression(paste(italic("A")[sat])),
    "GPP",
    expression(paste(italic("V")[cmax])),
    expression(paste(italic("J")[max])),
    expression(paste(italic("g")[s])),
...
gg_eco2_mesi <- df5 %>%
  ...
  labs(x = "", y = "Normalised log response ratio", size = expression(paste("Error"^{-1}))) +
  # ylim(-1, 1) +
  labs(subtitle = "MESI data", title = expression(paste("Response to eCO"[2]))) +
  scale_x_discrete("", 
                   labels = df_labels$mylabl) +
  coord_flip()

saveRDS(gg_eco2_mesi, file = paste0(here::here(), "/data/gg_eco2_mesi.rds"))

gg_eco2_mesi
ggsave(paste0(here::here(), "/fig/metaanalysis_eco2_mesi.pdf"), width = 8, height = 5)
ggsave(paste0(here::here(), "/fig/metaanalysis_eco2_mesi.png"), width = 8, height = 5)
```
...
gg_eco2_cpl_agb_bgb_mesi <- df5_agb_bnpp %>% 
  ...
  labs(title = " ",
       subtitle = " ",
       x = "Normalised log RR of belowground biomass", 
       y = "Normalised log RR of aboveground biomass", 
       size = expression(paste("Error"^{-1}))) +
  theme_classic()

saveRDS(gg_eco2_mesi, file = paste0(here::here(), "/data/gg_eco2_cpl_agb_bgb_mesi.rds"))

gg_eco2_cpl_agb_bgb_mesi
ggsave(paste0(here::here(), "/fig/eco2_cpl_agb_bgb_mesi.pdf"), width = 6, height = 5)
ggsave(paste0(here::here(), "/fig/eco2_cpl_agb_bgb_mesi.png"), width = 6, height = 5)
```
...
plot_grid(gg_eco2_mesi, gg_eco2_cpl_agb_bgb_mesi, rel_widths = c(1, 0.65), labels = c("a", "b"))
ggsave(paste0(here::here(), "/fig/eco2_mesi.pdf"), width = 12, height = 5)
ggsave(paste0(here::here(), "/fig/eco2_mesi.png"), width = 12, height = 5)
```
