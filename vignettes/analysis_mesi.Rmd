...
- Is information on fumigation type available in some way? I'd like to select data from FACE and open-top chambers. (-> KEVIN)
...
- No info on units in response variable. (-> KEVIN) Not needed for response ratio, but (critically) informative for understanding the data (e.g., selecting variables). For example, what are:
...
df2 <- df1 %>% 
  ...
  mutate(myvar = ifelse(myvar %in%  c("agb", "agb_c"),   "agb", myvar)) %>% 
  mutate(myvar = ifelse(myvar %in%  c("bgb", "bgb_c"),   "bgb", myvar)) %>% 
  mutate(myvar = ifelse(myvar %in%  c("lai", "lai_max"), "lai", myvar)) %>% 
  mutate(myvar = ifelse(myvar %in%  c("bgb", "fine_root_biomass"), "bgb", myvar)) %>% 
  mutate(myvar = ifelse(myvar %in%  c("root_production", "fine_root_production"),
                        "root_production", 
                        myvar)) %>% 
...
df4 %>%
  ...
  geom_crossbar( data = df_box, 
                 ...
                 fill = "tomato", color = "tomato4", alpha = 0.6, width = 0.5 
                 ) +
...
- No difference between (positive) aboveground biomass (`agb`) response and belowground biomass (`bgb`) response (but outlier in `agb`), but where root:shoot ratio observations available, indicating positive response (although not significant - few data points)
- Positive response in N uptake (but few data available only!)
- Decline in mineral N availability (but not significant)
- Strong (in two experiments extraordinarily strong) reduction in stomatal conductance (`gs`).
...
- Find out in which experiments, belowground biomass (`bgb`) was actually measured and not estimated based on aboveground biomass (or other aboveground observations) and scaling assumptions (-> STUDENT).
- Very little N uptake data. Wasn't there more in GCME (-> Beni)? Can we get more from published experiments in general (-> STUDENT)?
- Negative outliers for `agb` and `root_production` and positive outliers for `anpp`.
...
- Only one experiment providing an estimate of root:shoot ratio change - It's pointing towards a reduction.
- Unsurprisingly, inorganic soil N increases in some cases very strongly, but in one experiment not.

**Issues:**

- Get more data for root:shoot ratio from N-fertilisation experiments.
