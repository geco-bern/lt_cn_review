...
Number of observations (paired ambient and elevated) per response variable.
```{r}
df_sub %>% 
  group_by(response) %>% 
  summarise(n = n()) %>% 
...
Number of years per experiment.
```{r}
df_sub %>% 
...
df_sub2 <- df_sub %>% 
  filter(response == "agb" & exp %in% use_exp)
```
...
```{r}
metafor::forest(mod, xlab = "Log Response Ratio", mlab = "", xlim = c(-1,1), cex = 0.5)
```
